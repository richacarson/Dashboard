#!/usr/bin/env python3
"""
Q3 2026 Rebalance Executor
==========================

Performs the quarterly rebalance described in:
  public/research/IC Proposal Q3 2026 (Carson target sheet)
...
Workflow:
  ...
  Phase 2: Pull 7/09/26 close prices for all current + new holdings.
...
from scripts.q3_2026_rebalance.ic_targets import (
...
def analyze_sleeve(tx_file, sleeve_name=None):
    ...
    # CURRENT holdings source of truth = the app's portfolio-history JSON (a full
    # transaction replay that already reflects Q2 and the CTRA→DVN merger). The
    # 'By Security' header snapshot is stale (dated pre-Q2), so never trust it for
    # share counts — override from JSON when available.
    if sleeve_name:
        jpath = REPO / "public" / f"portfolio-history-{sleeve_name}.json"
        if jpath.exists():
            jh = (json.load(open(jpath)) or {}).get("holdings", {})
            if jh:
                shares = {t: s for t, s in jh.items() if s and s > 0.0001}
...
def run_sleeve(sleeve_name, tx_file, targets):
    ...
    # Pre-state
    pre = analyze_sleeve(tx_file, sleeve_name)
    ...
    # For 'By Security' sleeves (dividend), stock trades don't move cash — cash is
    # tracked purely via DEPOSIT/WITHDRAWAL. Deploy the excess cash down to the 1%
    # target with a single cash leg so post-rebalance cash lands exactly on target.
