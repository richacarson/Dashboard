...
- Do NOT ask questions or offer to help further` }],
  ...
  const [riskLoadDone, setRiskLoadDone] = useState(false);
  ...
  // Risk Sentinel — daily holdings risk read (classic Risk tab or terminal Risk drawer)
  useEffect(() => {
    if ((tab !== "risk" && tDrawer !== "risk") || riskFetched.current) return;
    riskFetched.current = true;
    const cb = `?v=${Math.floor(Date.now() / 60000)}`;
    fetch(`${import.meta.env.BASE_URL}risk-monitor.json${cb}`)
  ...
  const navItems = [
    ...
    { id: "risk", label: "Risk", icon: (a, onLight) => <svg width="21" height="21" viewBox="0 0 24 24" fill={a ? (onLight ? C.accentSoft : C.navAccentSoft) : "none"} stroke={a ? (onLight ? C.t1 : C.navText) : (onLight ? C.t4 : C.navTextMuted)} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg> },
    ...(deskOwner ? [{ id: "desk", label: "Desk", icon: (a, onLight) => <svg width="21" height="21" viewBox="0 0 24 24" fill={a ? (onLight ? C.accentSoft : C.navAccentSoft) : "none"} stroke={a ? (onLight ? C.t1 : C.navText) : (onLight ? C.t4 : C.navTextMuted)} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"/></svg> }] : []),
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "16px 24px" }}>
              ...
              {tDrawer === "settings" && (<div>
                ...
                <div style={{ marginBottom: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: C.t4, textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 6 }}>Account</div>
                  <div style={{ fontSize: 11, color: C.t4, marginBottom: 8 }}>{authEmail ? `Signed in as ${authEmail}` : "Signed in"}</div>
                  <button onClick={() => authSignOut()} style={{ padding: "8px 18px", border: `1px solid ${C.border}`, background: "transparent", color: C.t2, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Sign out</button>
                </div>
                <div style={{ marginBottom: 16 }}>
  ...
  return (
    <div ref={contentRef} onTouchStart={handleTabSwipeStart} onTouchEnd={handleTabSwipeEnd} style={{ minHeight: "100dvh", background: C.bg, color: C.t1, display: isDesktop ? "flex" : "block", paddingBottom: isDesktop ? 0 : 90, overflowY: "auto", fontFamily: theme === "terminal" ? "'IBM Plex Mono', 'SF Mono', 'Fira Code', 'Cascadia Code', Consolas, monospace" : undefined, letterSpacing: theme === "terminal" ? "-0.2px" : undefined, fontSize: theme === "terminal" ? "13px" : undefined }}>
      ...
      <div style={{ maxWidth: isDesktop ? 1400 : 960, margin: "0 auto", padding: isDesktop ? "0 40px" : "0 18px" }}>
        ...
        {tab === "desk" && <DeskTab C={C} isDesktop={isDesktop} />}

        {/* ━━━ OPPORTUNITIES ━━━ */}
        {tab === "risk" && <RiskView riskData={riskData} riskLoadDone={riskLoadDone} C={C} isDesktop={isDesktop} />}
