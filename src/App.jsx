...
- Do NOT ask questions or offer to help further` }],
  ...
  const { isOwner: deskOwner, session: deskSession } = useDeskSession();
  const tabIds = ["home", "risk", ...(( !deskSession || deskOwner) ? ["desk"] : []), "performance", "metrics", "charts", "news", "briefs", "research", "playbook", "screener", "opportunities", "settings"];
  ...
  // Risk Sentinel — daily holdings risk read
  useEffect(() => {
    if (tab !== "risk" || riskFetched.current) return;
