...
export default function RiskView({ riskData, riskLoadDone, C, isDesktop, terminal = false }) {
  const sevColor = (s) => s === "red" ? C.dn : s === "amber" ? C.warn : C.up;
  const sevSoft = (s) => s === "red" ? C.dnSoft : s === "amber" ? "rgba(217,119,6,0.12)" : C.upSoft;
  const d = riskData;
  // Terminal aesthetic: sharp corners, squared tags, denser. Classic: rounded cards/pills.
  const R = terminal ? 2 : 12;    // cards / rows
  const RP = terminal ? 2 : 999;  // pills / tags
  ...
  const pill = (label, n, s) => (
    <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "7px 13px", borderRadius: RP, background: sevSoft(s), border: `1px solid ${sevColor(s)}33` }}>
    ...
    <div style={{ animation: "fadeIn 0.3s ease", paddingTop: 20 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
        ...
        {typeof d.universe_count === "number" && (
          <div style={{ display: "flex", alignItems: "center", padding: "7px 13px", borderRadius: RP, background: C.surface, border: `1px solid ${C.border}`, fontSize: 11, color: C.t3, fontWeight: 600 }}>
      ...
      {/* Headline */}
      {d.summary?.headline && (
        <div style={{ padding: "12px 14px", borderRadius: R, background: C.card, border: `1px solid ${C.border}`, color: C.t2, fontSize: 13, lineHeight: 1.5, marginBottom: 16 }}>
          {d.summary.headline}
        </div>
      )}
      ...
      {d.roster_integrity && d.roster_integrity.ok === false && Array.isArray(d.roster_integrity.issues) && d.roster_integrity.issues.length > 0 && (
        <div style={{ padding: "12px 14px", borderRadius: R, background: C.dnSoft, border: `1px solid ${C.dn}55`, marginBottom: 16 }}>
      ...
      {!sorted.length ? (
        ...
        <div key={f.id || i} style={{ padding: "13px 15px", borderRadius: R, background: C.card, border: `1px solid ${C.border}`, borderLeft: `3px solid ${sevColor(f.severity)}`, marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, flexWrap: "wrap" }}>
            ...
            {f.category && <span style={{ fontSize: 10, color: C.t3, padding: "2px 8px", borderRadius: RP, background: C.surface, border: `1px solid ${C.border}`, textTransform: "uppercase", letterSpacing: 0.5 }}>{f.category}</span>}
      ...
      {Array.isArray(d.deal_watch) && d.deal_watch.length > 0 && (
        <div style={{ marginTop: 18 }}>
          ...
          {d.deal_watch.map((dw, i) => (
            <div key={i} style={{ padding: "11px 14px", borderRadius: R, background: C.card, border: `1px solid ${C.border}`, marginBottom: 8 }}>
      ...
      {Array.isArray(d.levels) && d.levels.length > 0 && (
        <div style={{ marginTop: 18 }}>
          ...
          {d.levels.map((lv, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", borderRadius: R, background: C.card, border: `1px solid ${C.border}`, marginBottom: 6, fontSize: 12.5 }}>
      ...
      {Array.isArray(d.calendar) && d.calendar.length > 0 && (
        <div style={{ marginTop: 18, marginBottom: 20 }}>
          ...
          {d.calendar.map((ev, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", borderRadius: R, background: C.card, border: `1px solid ${C.border}`, borderLeft: `3px solid ${sevColor(ev.severity)}`, marginBottom: 6, fontSize: 12.5 }}>
